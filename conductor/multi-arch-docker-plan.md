...
**Goal:** 优化 GitHub Actions 工作流，利用 Docker 官方的 `buildx` 与 `qemu` 功能支持构建并发布多架构（异构）镜像（如 `linux/amd64` 和 `linux/arm64`），全面提升容器镜像的兼容性。同时优化 Dockerfile 以支持将外部版本号注入二进制文件。
...
2. 将该参数通过 `-ldflags="-X main.version=${VERSION}"` 注入编译步骤，确保 Docker 容器内运行的程序也能打印正确的版本号。
...
4. **双标签推送**：打 tag 发布时，不仅推送 `${TAG_NAME}` 标签（如 `v1.0.0`），还会同步更新 `latest` 标签，这是开源项目的最佳实践。
