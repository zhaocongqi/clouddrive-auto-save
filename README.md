...
魔法变量能自动从原始文件名中提取关键信息，让整理变得极其简单：

| 变量 | 提取示例 | 说明 |
| :--- | :--- | :--- |
| `{TASKNAME}` | `功夫熊猫` | 您在创建任务时填写的显示名称 |
| `{DATE}` | `20240420` | 自动识别文件名中的日期并清洗为纯数字 |
| `{YEAR}` | `2024` | 自动提取 4 位年份数字 |
| `{CHINESE}` | `功夫熊猫` | 自动提取文件名中的连续中文 |
| `{EXT}` | `mp4` | 自动保留原始后缀名 |

**推荐配置示例：**
*   **替换规则**: `[{DATE}] {TASKNAME}.{EXT}`
*   **效果**: `[极速版] 2024.04.20 功夫熊猫4.Kung.Fu.Panda.4.mp4`  ➡️  `[20240420] 功夫熊猫4.mp4`
